-- BRC Build 04.30: add Belt / Foil Calculator to per-user Tool access.
-- Cumulative from 04.28: also accepts fractionchart if 04.29 migration was not run separately.
do $$
declare r record;
begin
  for r in
...
alter table public.user_tool_settings
  ...
  check (tool_id in ('belt','calculator','converter','wire','thread','fractionchart'));

insert into public.user_tool_settings(user_id,tool_id,allowed,sort_order)
select p.id,t.tool_id,true,t.sort_order
from public.profiles p
cross join (values ('belt',5),('fractionchart',50)) as t(tool_id,sort_order)
...
begin
  ...
  foreach v_tool in array p_tool_ids loop
    if v_tool not in ('belt','calculator','converter','wire','thread','fractionchart') then continue; end if;
