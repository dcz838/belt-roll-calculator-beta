...
const i18n = {
  en: {
    ...
    clearInputs: "Clear",
    about: "About",
    note: "Note: Calculations are estimates only.",
    done: "Done",
    installApp: "Install App",
    installUnavailable: "Install instructions are shown in About.",
    designed: "Designed and Developed by",
    copyright: "Copyright © 2026 Raymond Lei",
    ...
    installTitle: "Install to Device",
    ...
    estimatedOD: "Estimated OD",
    copied: "Result copied.",
    warningNumber: "Wrong input: Please enter numbers only.",
    warningThickness: "Thickness must be greater than 0.",
    warningCore: "Core Diameter must be greater than 0.",
    warningPositive: "Remaining value must be greater than 0.",
    warningODCore: "Remaining OD must be greater than Core Diameter."
  },
  es: {
    version: "Edición Web 2.1 Beta",
    input: "Datos",
    result: "Resultado",
    units: "Unidades",
    thickness: "Espesor",
    ...
    clearInputs: "Borrar",
    about: "Acerca de",
    note: "Nota: Los cálculos son estimados.",
    done: "Listo",
    installApp: "Instalar",
    installUnavailable: "Las instrucciones de instalación están en Acerca de.",
    designed: "Diseñado y desarrollado por",
    copyright: "Copyright © 2026 Raymond Lei",
    rights: "Todos los derechos reservados.",
    legal: "No part of this application, including its design, calculations, source code, documentation, images, or data structure, may be copied, modified, distributed, or reproduced without written permission from Raymond Lei.",
    installTitle: "Instalar en dispositivo",
    installIOS: "Safari → Compartir → Agregar a pantalla de inicio",
    ...
    warningNumber: "Entrada incorrecta: ingrese solo números.",
    warningThickness: "El espesor debe ser mayor que 0.",
    warningCore: "El diámetro del núcleo debe ser mayor que 0.",
    warningPositive: "El valor debe ser mayor que 0.",
    warningODCore: "El diámetro exterior restante debe ser mayor que el diámetro del núcleo."
  },
  zh: {
    version: "网页版 2.1 Beta",
    input: "输入",
    result: "结果",
    units: "单位",
    thickness: "皮带厚度",
    ...
    installApp: "安装应用",
    installUnavailable: "安装说明已显示在 About 页面。",
    designed: "设计与开发",
    copyright: "版权所有 © 2026 Raymond Lei",
    rights: "保留所有权利。",
    legal: "未经 Raymond Lei 书面授权，不得复制、修改、分发、转载本应用程序的设计、计算逻辑、源代码、文档、图片或数据结构。",
    installTitle: "安装到设备",
    installIOS: "Safari → 分享 → 添加到主屏幕",
    ...
    estimatedOD: "预估外径",
    copied: "结果已复制。",
    warningNumber: "错误：请输入有效数字。",
    warningThickness: "错误：皮带厚度必须大于 0。",
    warningCore: "错误：卷芯直径必须大于 0。",
    warningPositive: "错误：数值必须大于 0。",
...
const el = {
  ...
  remainingUnit: document.getElementById("remainingUnit"),
  warningArea: document.getElementById("warningArea"),
  resultArea: document.getElementById("resultArea"),
  lengthUnitLabel: document.getElementById("lengthUnitLabel"),
  lengthFeetLabel: document.getElementById("lengthFeetLabel"),
  lengthMetersLabel: document.getElementById("lengthMetersLabel"),
  lengthUnitValue: document.getElementById("lengthUnitValue"),
  lengthFeetValue: document.getElementById("lengthFeetValue"),
  lengthMetersValue: document.getElementById("lengthMetersValue"),
...
function formatNumber(value, digits = 3) {
  ...
  return n.toLocaleString(undefined, {
    ...
    minimumFractionDigits: n < 10 ? Math.min(3, digits) : 0
  });
}
...
function setUnits(units) {
  ...
  document.getElementById("unitMM").classList.toggle("active", units === "MM");
  document.getElementById("unitInch").classList.toggle("active", units === "Inch");

  document.querySelectorAll("[data-current-unit]").forEach(node => {
    node.textContent = units === "MM" ? "mm" : "in";
  });
...
function setMode(mode) {
  ...
  document.getElementById("modeOD").classList.toggle("active", mode === "od");
  document.getElementById("modeTurns").classList.toggle("active", mode === "turns");
...
function calculate() {
  ...
  if (Number.isNaN(thickness) || Number.isNaN(core) || Number.isNaN(remaining)) {
    ...
    warning = t("warningThickness");
  } else if (core <= 0) {
    warning = t("warningCore");
  } else if (remaining <= 0) {
    warning = t("warningPositive");
  } else if (state.mode === "od" && remaining <= core) {
    warning = t("warningODCore");
  }
...
async function installApp() {
  ...
  openAbout();
  const card = document.getElementById("installInstructions");
  if (card) {
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-unit]").forEach(button => {
    button.addEventListener("click", () => setUnits(button.getAttribute("data-unit")));
  });
  ...
  window.addEventListener("beforeinstallprompt", event => {
    ...
    /* install button already visible */
  });

  window.addEventListener("appinstalled", () => {
    state.deferredInstallPrompt = null;
    if (el.installButton) /* install button remains visible */
  });
}
