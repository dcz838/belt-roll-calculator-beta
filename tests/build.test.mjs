...
test("build metadata is current and no stale sidebar version remains",()=>{assert.equal(version.build,"2026.08.13.04.07");assert.match(html,/sidebarVersion/);assert.doesNotMatch(html,/2026\.07\.27\.03\.07/);assert.doesNotMatch(html,/Web Edition 2\.2 Beta/)});
...
test("GitHub Pages assets are cache-busted",()=>{assert.match(html,/cloud\.js\?v=202608130407/);assert.match(html,/app\.js\?v=202608130407/);assert.match(sw,/cache:'no-store'/)});
...
import { test as startupTest } from 'node:test';
startupTest('app startup is self-contained and not blocked by core.mjs loading',()=>{assert.doesNotMatch(app,/^import\{/m);assert.match(html,/<script src=\"js\/app\.js\?v=202608130407\"><\/script>/);});
